...
- **Security**: TLS provides confidentiality; the shared secret provides authentication (mutual HMAC challenge-response inside the TLS channel). The proof is **bound to the TLS certificate** — an on-path attacker without the secret can't man-in-the-middle by terminating TLS on each side and relaying the handshake, because each end binds the server certificate it actually saw and the relayed proof no longer verifies. The self-signed certificate is auto-generated on first startup and stored at `<app dir>/colibra/<ServiceId>.pfx` — per-service, so multiple services on one machine never collide. Point `CertificatePath` elsewhere (or pre-provision your own PFX) if you prefer. (The channel-bound handshake is protocol v2, not wire-compatible with v0.4.x nodes — upgrade the whole cluster together.)
